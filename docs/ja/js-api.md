...
- パラメータ名: 一意である必要があり、ASCII の大小文字を畳み込んだ後も一意でなければなりません。`os` と `OS` は共存できません。
- 値とエイリアス: 1 つのパラメータの中で、値とそのすべてのエイリアスが、ASCII の大小文字を畳み込んだ後も互いに異なっている必要があります。どちらの規則も、値の解決が大文字小文字を区別しないためのものです。大小文字だけが異なると `os = WINDOWS` の指す先が一意に定まりません。
- `weights`: 各重みは `0` より大きい有限数である必要があります。
