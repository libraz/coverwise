...
| パフォーマンス | ペアワイズでは同等、高強度では優位 | ペアワイズでは同等 |
| 依存 | WASM対応が必要 | なし |
| API | 同一 | 同一 |
...
タプル数とテスト数は、同梱ジェネレータを `seed: 42` で実行した結果です。[ドキュメント契約テスト](tests/integration/docs_contract_test.cpp) が実行のたびに再生成するため、アルゴリズムを変更した場合は表が古びる代わりにテストが失敗します。理論下限は直交配列 (OA) 理論または v² 下界に基づく既知の下限値です。貪欲法アルゴリズムは一般に理論最小値の 1.5〜2.5 倍のテスト数を生成します。ネイティブ C++・WASM・Pure TS は xoshiro128** と SplitMix32 シード、棄却サンプリングを共有するため、有効な入力とシードが同じなら、エンジン間で同一のテストスイートを生成します。

実行時間は構成ごとには掲載していません。ホスト・ランタイム・ビルドに左右されるうえ、このリポジトリからは再現できない数値だからです。傾向としては、ペアワイズでは 2 つの JavaScript エンジンはほぼ同等で、強度とタプル数が大きくなるほど WASM エンジンが優位になります。所要時間が問題になる場合は、対象の構成を自分のハードウェアで計測してください。
